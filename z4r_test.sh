...
get_repo() {
 ...
 for listfile in cloudflare-ipset.txt cloudflare-ipset_v6.txt netrogat.txt russia-discord.txt russia-youtube-rtmps.txt russia-youtube.txt russia-youtubeQ.txt tg_cidr.txt; do curl -L -o /opt/zapret/lists/$listfile https://raw.githubusercontent.com/kostazol/zapret4rocket/improving_policy/lists/$listfile; done
...
echo -e "${yellow}zeefeer обновлен (UTC +0): $(curl -s "https://api.github.com/repos/kostazol/zapret4rocket/commits?path=z4r.sh&sha=improving_policy&per_page=1" | grep '"date"' | head -n1 | cut -d'"' -f4) ${plain}"
